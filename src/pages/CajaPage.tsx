...
export default function CajaPage() {
  ...
  const closeMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["active-shift"] }); setShowCloseModal(false); },
  });
    ...
    <div className="flex flex-col gap-6">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{activeShift ? "Turno activo" : "Sin turno abierto"}</p>
        </div>
        ...
        {activeShift && (
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => { setCloseForm({ closingBalance: expectedBalance }); setShowCloseModal(true); }}
              className="px-4 py-2.5 rounded-xl bg-surface-800 hover:bg-surface-700 border border-surface-700 text-white text-sm font-medium transition-colors">
              Cerrar Turno
            </button>
          </div>
        )}
      ...
      {shiftLoading ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          className="flex flex-col items-center justify-center bg-surface-900 border border-surface-800 rounded-2xl py-20 gap-4">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: "Balance Apertura", value: fmt(activeShift.openingBalance), color: "text-white" },
              ...
              <div key={k.label} className="bg-surface-900 border border-surface-800 rounded-2xl p-4">
      ...
      <Modal open={showOpenModal} onClose={() => setShowOpenModal(false)} title="Abrir Turno" maxWidth="max-w-sm">
        <form onSubmit={(e) => { e.preventDefault(); openMutation.mutate(openForm); }} className="flex flex-col gap-4">
          ...
          <div className="flex justify-end gap-3 pt-1">
            <button type="button" onClick={() => setShowOpenModal(false)} className="px-4 py-2 rounded-lg text-sm text-slate-400 hover:text-white hover:bg-surface-800">Cancelar</button>
        ...
        <form onSubmit={(e) => { e.preventDefault(); closeMutation.mutate(closeForm); }} className="flex flex-col gap-4">
          ...
          {activeShift && (
            <p className="text-xs text-slate-500">
              Balance esperado: <span className="text-white">{fmt(expectedBalance)}</span>
          ...
          <div className="flex justify-end gap-3 pt-1">
            <button type="button" onClick={() => setShowCloseModal(false)} className="px-4 py-2 rounded-lg text-sm text-slate-400 hover:text-white hover:bg-surface-800">Cancelar</button>
      ...
      <Modal open={showTxModal} onClose={() => setShowTxModal(false)} title="Nueva Transacción" maxWidth="max-w-sm">
        <form onSubmit={(e) => { e.preventDefault(); txMutation.mutate(txForm); }} className="flex flex-col gap-4">
          ...
          <div className="flex justify-end gap-3 pt-1">
            <button type="button" onClick={() => setShowTxModal(false)} className="px-4 py-2 rounded-lg text-sm text-slate-400 hover:text-white hover:bg-surface-800">Cancelar</button>
